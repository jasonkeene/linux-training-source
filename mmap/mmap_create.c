...
main(int argc, char *argv[])
{
    ...
    for (int j = 1; j < argc; j++) {
        ...
        touch = 0;
        factor = 1;
        while (isalpha((unsigned char) *ap)) {
            switch (*ap) {
            ...
            default:    fatal("Bad flag '%c' in \"%s\"", *ap, argv[j]);
            }
            ap++;
        }

        size = strtoul(ap, NULL, 0) * factor;
        ...
        if (touch) {
            ...
            for (char *p = addr; p < addr + size; p += 1024)
                *p = '\0';
        }
    }
