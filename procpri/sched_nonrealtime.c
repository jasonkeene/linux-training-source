...
static void
setSchedPolicy(int childNum, char *arg)
{
...
main(int argc, char *argv[])
{
    ...
    for (int childNum = 1; childNum < argc; childNum++) {
        switch (fork()) {
        case -1:
            errExit("fork");
        ...
        case 0:
            ...
            setSchedPolicy(childNum, argv[childNum]);
    ...
    for (;;) {
        ...
        /* Yes, the following code is racy. It might happen that an element
           of the array is changed while we perform the steps below, with the
           consequence that we may see incorrect statistics occasionally. */
